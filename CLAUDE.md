...
| Box informativi configurabili (`wm-config-detail`) | oc:8181 | `config-detail/*` (nuovo), `types/config.ts`, `wm-core.module.ts`, `home/home-layer/home-layer.component.html`, `track-properties/track-properties.component.html` | Accordion custom (non `ion-accordion`), apertura esclusiva, paginazione 10+"Mostra altro/meno", lazy `innerHTML` embed, iframe responsive senza `aspect-ratio` forzato. Wiring EcPoi nel repo principale. Dettagli in `docs/features/8181-schermata-cammino-con-blocchi-informativi-configurabili/notes.md` |
