...
| Box informativi configurabili (`wm-config-detail`) | oc:8181 | `config-detail/*` (nuovo), `types/config.ts` (`ILAYER.config_detail`), `wm-core.module.ts`, `home/home-layer/home-layer.component.html`, `track-properties/track-properties.component.html`; tipi in `wm-types/src/config.ts` | Accordion custom (non `ion-accordion`), apertura esclusiva, paginazione 10+"Mostra altro/meno", lazy `innerHTML` embed, iframe responsive senza `aspect-ratio` forzato. Tipi condivisi `ConfigDetailBox*` in `@wm-types/config`. Wiring EcPoi nel repo principale. Dettagli in `docs/features/8181-schermata-cammino-con-blocchi-informativi-configurabili/notes.md` |
