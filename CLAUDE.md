...
| Condivisione percorso registrato sui social — pulsante e stato UI | oc:8183 | `ugc-track-properties.component.ts`/`.html`/`.scss`, `types/eugc-track-share-state.enum.ts` (nuovo), `localization/i18n/{it,en,de,es,fr,pr,sq}.ts`, `ugc-track-properties.component.spec.ts` (nuovo) | Solo pulsante e stato UI (idle/generating/success/error); screenshot e plugin nativo restano in map-core/webmapp-app. Nuovo `@Output('share-track')` emette la traccia, nuovo `@Input('shareResult')` riceve l'esito. Dettagli completi in `docs/features/8183-condivisione-percorso-registrato-sui-social/notes.md` |
...
- **Guardia doppio tap a due livelli**: `triggerShare()` non fa nulla se lo stato è già `GENERATING` (guardia in TS) oltre al `[disabled]` sul bottone nel template (già previsto dal task 5) — la guardia in map-core sul componente mini-map resta comunque la seconda linea di difesa
- **Nessun default client-side per `OPTIONS.ugcTrackShareEnabled`** in `conf.reducer.ts` (a differenza di `showTrackRemainingDistance`, oc:8177, che aveva un default temporaneo `true` per i test): il flag resta `undefined` finché un backend non lo abilita esplicitamente via `config.json` — scelta deliberata per una feature nuova che invoca un plugin nativo, non ancora pronta ovunque
- **Stato `SUCCESS` senza auto-reset temporizzato**: resta visibile finché il pannello non si richiude/riapre o l'utente non ritenta — evita di introdurre un timer/subscription solo per un dettaglio di UX; punto aperto per eventuale revisione UX
- **Test come istanza TS pura, non `TestBed`**: `ugc-track-properties.component.spec.ts` costruisce il componente con `new UgcTrackPropertiesComponent(storeSpy, alertCtrlSpy, langSvcSpy, urlHandlerSvcSpy)`, bypassando la compilazione del template Angular — evita il crash `NG0201` per `APP_TRANSLATION` mancante in DI già documentato sopra (oc:8023) per gli spec di componenti. Verificato comunque che il template compila e supera l'intera suite (`ng test wm-core`, 174/174 verde) con Node 20 + Chrome headless, prima di lasciare solo il test isolato in CI
