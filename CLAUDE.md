...
| Etichetta generica dislivello su percorsi ad anello | oc:8493 | `tab-detail/tab-detail.component.html`, `localization/i18n/*` (nuova chiave `slope`) | Su roundtrip la label passa da `ascent`/"Dislivello positivo" a `slope`/"Dislivello" (generico), perché su un anello positivo e negativo coincidono e la riga `descent` non viene mai mostrata per i roundtrip. `ugc-track-data.component.html` (tracce UGC) esplicitamente non toccato. Dettagli completi in `docs/features/8493-etichetta-dislivello-anello/notes.md` |

## Decisioni architetturali

### Etichetta generica dislivello su percorsi ad anello (oc:8493)
- **Fix di sola etichetta, nessun cambio di dato**: il valore mostrato resta sempre `properties.ascent`; su roundtrip cambia solo la chiave i18n usata (`slope` invece di `ascent`), non il numero.
- **Nuova chiave `slope` inizialmente tradotta copiando la parola già usata da `pendenza`** (gradiente % istantaneo nel grafico altimetrico, `slope-chart.component.html`) in 5 lingue su 7 (en/de/es/fr/sq) — trovato in Fase: challenge (review adversariale), reintroduceva la stessa ambiguità terminologica che il ticket voleva risolvere. Corretto allineando `slope` alla radice già usata da `ascent`/`descent` in ciascuna lingua: de→`Höhenunterschied`, es→`Desnivel`, fr→`Dénivelé`, pr→`Elevação` (radice comune a `Elevação +`/`Elevação -`), sq→`Lartësia`. it (`Dislivello`) ed en (`Slope`) restano invariati perché già coerenti con la propria radice `ascent`/`descent`.
- **Inglese lascia una collisione preesistente non risolta**: `pendenza`="Slope" e `ascent`="Slope +" condividevano già la parola "Slope" prima di questo ticket — non aggravato da questo fix, ma non risolto (avrebbe richiesto toccare `ascent`/`descent` esistenti, fuori scope per un fix di etichetta su un solo caso).
- **`ugc-track-data.component.html`** (tracce UGC registrate via GPS) mostra sempre "ascent" incondizionatamente, senza gestione `roundtrip` — esplicitamente fuori scope, contesto diverso (tracce utente vs layer editoriali).
- **Nessuna guardia runtime sull'invariante `ascent≈descent`**: se un layer è marcato `roundtrip:true` per errore editoriale, l'etichetta generica implica erroneamente equivalenza — rischio accettato, il flag `roundtrip` è responsabilità del backoffice/editor, non validato lato frontend.
...
- **Fix post-merge: il filtro non raggiungeva mai la mappa** — `routeFilters` era letto solo da `filteredLayers$`/`confHOMEFiltered` (lista/griglia Home), mai dal rendering OpenLayers (submodule `map-core`, che usa un sistema di filtro preesistente e distinto, `mapFilters`/`filterTracks`). Nuovo selettore `routeFilteredLayerIds` (`user-activity.selector.ts`) incluso in `mapFilters`, consumato da `styleFn` in `map-core` (vedi CLAUDE.md di quel repo).
- **Fix post-merge: aprendo un layer specifico da una lista già filtrata, le sue tappe sparivano ovunque (mappa, lista, e il tab "Tappe" stesso non compariva)** — `showResultTabSelected$` forzava il tab `'layers'`/`null` in base a `hasActiveRouteFilters` senza considerare il caso "layer già aperto", dove i filtri Home non sono più pertinenti. Fix: condizione `&& !currentLayer` in `home-result.component.ts`, più l'analogo aggiustamento nei bottoni tab del template (`home-result.component.html`) e in `styleFn` (map-core). Dettagli completi in `docs/features/8414-filtri-cammini-home/notes.md`.
...
- **Bug di regressione trovato e corretto**: nello split di `home-layer.component.scss` (sopra), le regole di overlay `.wm-box-title` (titolo bianco in bottom-overlay sulla foto) e `wm-img.wm-home-layer-logo-overlay` (badge logo 56×56 in basso a destra) erano finite **solo** nel design della variante `.camminiditalia` (grid `fascia bianca`) e non erano mai state riportate in `home-layer-shared.scss` né lasciate nel `home-layer.component.scss` di default — erano invece parte del design generico usato da **tutti** gli shard fin da oc:8164/oc:8176, non una specificità camminiditalia. Effetto: il componente di default (tutti gli shard tranne camminiditalia buildato con `--configuration=camminiditalia`) perdeva completamente lo stile — titolo renderizzato come testo semplice sotto la foto, logo come immagine non dimensionata/posizionata. Fix: le due regole sono state ripristinate (identiche alla versione pre-oc:8391) direttamente in `home-layer.component.scss`, lasciando `home-layer-shared.scss` con le sole regole realmente comuni (heart preferiti).
